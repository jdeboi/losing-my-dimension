...
import { Scene, AnaglyphUniversalCamera, UniversalCamera, HemisphericLight, Vector3, SceneLoader,AssetContainer, MeshBuilder, StandardMaterial, VideoTexture, CubeTexture, Color3,Color4, Mesh, Texture } from 'babylonjs';
...
const onSceneReady = scene => {
  ...
  url = process.env.PUBLIC_URL + "/models/monitor/"
  ...
  SceneLoader.LoadAssetContainer(url, "QueenPalmTree.obj", scene, function (container) {
    ...
    let copyL2 = container.instantiateModelsToScene().rootNodes[1];
    copyL2.rotation.y = Math.PI*1.5;
    copyL2.position.x = 8;
    copyL2.position.z = -6;
    trees.push(copyL2);

    let copyR2 = container.instantiateModelsToScene().rootNodes[1];
    copyR2.rotation.y = Math.PI*1.5;
    copyR2.position.x = -8;
    copyR2.position.z = -6;
    trees.push(copyR2);

    let copyL3 = container.instantiateModelsToScene().rootNodes[1];
    copyL3.rotation.y = Math.PI*1.5;
    copyL3.position.x = -10;
    copyL3.position.z = -8;
    trees.push(copyL3);

    let copyR3 = container.instantiateModelsToScene().rootNodes[1];
    copyR3.rotation.y = Math.PI*1.5;
    copyR3.position.x = 10;
    copyR3.position.z = -8;
    trees.push(copyR3);
  });

  // url = process.env.PUBLIC_URL + "/models/palm/"
  // SceneLoader.Append(url, 'QueenPalmTree.obj', scene, function (scene) {
  //   // scene.meshes[scene.meshes.length-3].scaling = new Vector3(5, 5, 5);
  //   scene.meshes[scene.meshes.length-3].position.x = -5;
  //   scene.meshes[scene.meshes.length-3].position.y = -5;
  //   scene.meshes[scene.meshes.length-3].position.z = -2;
  // });

  // SceneLoader.Append(url, 'QueenPalmTree.obj', scene, function (scene) {
  //   // scene.meshes[scene.meshes.length-1].scaling = new Vector3(5, 5, 5);
  //   scene.meshes[scene.meshes.length-1].position.x = 5;
  //   scene.meshes[scene.meshes.length-1].position.y = -5;
  //   scene.meshes[scene.meshes.length-1].position.z = -2;
  // });
  ...
  myPlaneB.material.diffuseTexture = new Texture("/textures/yogamat_gr.png", scene);
  ...
  skyboxMaterial.reflectionTexture = new CubeTexture("/textures/galaxy", scene);
...
const videoBackgroundURL = process.env.PUBLIC_URL + "/videos/swings.mp4";
  ...
  function getFrame(dimW, dimH, dimX, dimY, tit, vid) {
    return (
      <Frame title={tit} content={
        ...
        url={process.env.PUBLIC_URL + "/videos/" + vid}
        />
      }
