    ...
    if (props.origins) {
      if (ps.length == 0) {
        for (let i = 0; i < props.origins.length; i++) {
          ...
          ps[i] = new ParticleSystem(p, p.createVector(x, y), props.velocities[i].velX, props.velocities[i].velY, props.degrees[i]);
          ps[i].initParticles(80);
        }
      }
      else {
...
function ParticleSystem(p, position, velX, velY, deg) {
  this.p5 = p;
  this.particles = [];
  this.numParticle = 0;
